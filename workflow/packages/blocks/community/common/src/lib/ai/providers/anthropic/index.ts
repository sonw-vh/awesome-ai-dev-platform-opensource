    ...
    return {
        ...
        function: {
            call: async (params) => {
                ...
                if (params.files.length) {
                    ...
                    if (contents.length) {
                        ...
                        if (lastMessage && lastMessage.role === AIChatRole.USER) {
                            const exitingContent = lastMessage.content as Array<Anthropic.Messages.ContentBlockParam>;
                ...
                const toolCallsResponse = completion.content.filter((choice): choice is ToolUseBlock => choice.type === 'tool_use');
