...
function createTriggerTemplate(displayName: string, description: string, technique: string) {
    ...
    if (technique === 'polling') {
        triggerTemplate = `
import { createTrigger, TriggerStrategy, PiecePropValueSchema  } from 'workflow-blocks-framework';
import { DedupeStrategy, Polling, pollingHelper } from 'workflow-blocks-common';
...
const createTrigger = async (blockName: string, displayTriggerName: string, triggerDescription: string, triggerTechnique: string) => {
...
export const createTriggerCommand = new Command('create')
    ...
    .action(async () => {
        ...
        createTrigger(answers.blockName, answers.triggerName, answers.triggerDescription, answers.triggerTechnique);
    });
