...
import { AppSystemProp, WorkerSystemProp } from 'workflow-server-shared'
import { PrincipalType, WorkerMachineHealthcheckRequest, WorkerMachineHealthcheckResponse, WorkerPrincipal } from 'workflow-shared'
...
export const workerMachineController: FastifyPluginAsyncTypebox = async (app) => {
    ...
    app.post('/heartbeat', HeartbeatParams, async (request) => {
        ...
        const response: WorkerMachineHealthcheckResponse = {
            ...
            BLOCKS_SOURCE: system.getOrThrow(AppSystemProp.BLOCKS_SOURCE),
            DEV_BLOCKS: system.get(AppSystemProp.DEV_BLOCKS)?.split(',') ?? [],
