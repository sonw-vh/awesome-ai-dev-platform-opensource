...
import { LATEST_JOB_DATA_SCHEMA_VERSION, QueueName, RepeatableJobType, ScheduledJobData } from 'workflow-server-shared'
import { ExecutionType, isNil, RunEnvironment, ScheduleType } from 'workflow-shared'
...
async function updateLegacyRepeatableJobKey(repeatableJobs: RepeatableJob[], log: FastifyBaseLogger) {
    ...
    for (const repeatableJob of repeatableJobs) {
        ...
        await client.del(`aixblock:repeatJobKey:${flowVersionId}`)
        count++
    }
    log.info({
        jobs: count,
