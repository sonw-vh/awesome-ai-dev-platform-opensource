...
import { AppSystemProp, exceptionHandler, JobType, QueueName } from 'workflow-server-shared'
import { AIxBlockError, ApId, ErrorCode, isNil } from 'workflow-shared'
...
export const redisQueue = (log: FastifyBaseLogger): QueueManager => ({
    ...
    async removeRepeatingJob({ flowVersionId }: { flowVersionId: ApId }): Promise<void> {
        ...
        if (!result) {
            exceptionHandler.handle(new AIxBlockError({
