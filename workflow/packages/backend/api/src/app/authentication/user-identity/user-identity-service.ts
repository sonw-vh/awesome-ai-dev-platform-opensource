...
import { AIxBlockError, apId, ErrorCode, isNil, UserIdentity } from 'workflow-shared'
...
export const userIdentityService = (log: FastifyBaseLogger) => ({
    async create(params: Pick<UserIdentity, 'email' | 'password' | 'firstName' | 'lastName' | 'trackEvents' | 'newsLetter' | 'provider' | 'verified' | 'token' | 'url'>): Promise<UserIdentity> {
        log.info({
            email: params.email,
    ...
    async updateToken(params: UpdateTokenParams): Promise<void> {
        await userIdentityRepository().update(params.id, {
            token: params.token,
        })
    },
    async updateUrl(params: UpdateUrlParams): Promise<void> {
        await userIdentityRepository().update(params.id, {
            url: params.url,
        })
    },
    async deleteById(id: string) {
        await userIdentityRepository().delete({ id });
    }
})
