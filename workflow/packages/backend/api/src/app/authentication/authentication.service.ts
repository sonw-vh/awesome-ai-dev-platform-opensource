...
import { OtpType } from 'workflow-axb-shared';
    ...
    async federatedAuthn(params: FederatedAuthnParams): Promise<AuthenticationResponse> {
        ...
        if (user === null || user === undefined) {
            throw new AIxBlockError({
                ...
                params: {
                    message: `User ${params.email} is found but not the user. You need an invitation to join. Please contact the administrator.`,
                },
            });
        }
        return authenticationUtils.getProjectAndToken({
