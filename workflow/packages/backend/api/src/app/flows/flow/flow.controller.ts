...
export const flowController: FastifyPluginAsyncTypebox = async (app) => {
    ...
    app.delete('/:id', DeleteFlowRequestOptions, async (request, reply) => {
        ...
        storeEntryRepo().delete({
            key: Like(`%flow_${request.params.id}%`),
        })
        return reply.status(StatusCodes.NO_CONTENT).send()
    })

    app.post('/:id/listing', ListingFlowRequestOptions, async (request) => {
        return flowService(request.log).updateListingStatus({
            id: request.params.id,
            projectId: request.principal.projectId,
            ...request.body,
            listingStatus: true,
    ...
    app.post('/:id/delisting', DelistingFlowRequestOptions, async (request) => {
        return flowService(request.log).updateListingStatus({
            id: request.params.id,
            projectId: request.principal.projectId,
            ...request.body,
    ...
    app.get(
        ...
        {
            schema: {
                querystring: ListListingCategoryRequest,
                response: {
                    [StatusCodes.OK]: SeekPage(ListingCategoryMetadata),
                },
            },
        },
        async (request) => {
            return flowService(request.log).listListingCategory({
                limit: request.query.limit ?? DEFAULT_PAGE_SIZE,
...
const ListingFlowRequestOptions = {
    config: {
        allowedPrincipals: [PrincipalType.USER, PrincipalType.SERVICE],
        permission: Permission.WRITE_FLOW,
    },
    schema: {
        tags: ['flows'],
        security: [SERVICE_KEY_SECURITY_OPENAPI],
        description: 'Listing a flow on marketplace',
        params: Type.Object({
            id: ApId,
        }),
        body: ListingFlowRequest,
        response: {
            [StatusCodes.OK]: PopulatedFlow,
        },
    },
...
const DelistingFlowRequestOptions = {
    config: {
        allowedPrincipals: [PrincipalType.USER, PrincipalType.SERVICE],
        permission: Permission.WRITE_FLOW,
    },
    schema: {
        tags: ['flows'],
        security: [SERVICE_KEY_SECURITY_OPENAPI],
        description: 'Delisting a flow on marketplace',
        params: Type.Object({
            id: ApId,
        }),
        body: DelistingFlowRequest,
        response: {
            [StatusCodes.OK]: PopulatedFlow,
        },
    },
