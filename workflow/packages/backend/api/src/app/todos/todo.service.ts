...
import { AIxBlockError, apId, assertNotNullOrUndefined, Cursor, ErrorCode, FlowId, isNil, OffsetPaginationParams, PlatformId, ProjectId, SeekPage, spreadIfDefined, StatusOption, Todo, TodoWithAssignee, UNRESOLVED_STATUS, UserId } from 'workflow-shared'
import { repoFactory } from '../core/db/repo-factory'
import { Order } from '../helper/pagination'
...
export const todoService = (_log: FastifyBaseLogger) => ({
    ...
    async list(params: ListParams): Promise<SeekPage<TodoWithAssignee>> {
        const paginator = buildOffsetPaginator<Todo>({
            entity: TodoEntity,
            query: {
                limit: params.limit,
                page: params.page,
        ...
        return paginationHelper.createPageOffet<TodoWithAssignee>(enrichedData, pageCount)
    },
})
