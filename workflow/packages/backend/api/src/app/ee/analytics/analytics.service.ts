...
import { AnalyticsPieceReportItem, AnalyticsProjectReportItem, AnalyticsReportResponse, BlockCategory, flowPieceUtil, FlowStatus, PlatformId, PopulatedFlow, ProjectId } from 'workflow-shared'
...
async function numberOfFlowsWithAI(log: FastifyBaseLogger, flows: PopulatedFlow[], platformId: PlatformId) {
    const aiPiecePromises = flows.flatMap(flow => {
        const usedPieces = flowPieceUtil.getUsedBlocks(flow.version.trigger)
        return usedPieces.map(piece => blockMetadataService(log).getOrThrow({
            name: piece,
            version: undefined,
    ...
    return pieceMetadataList.filter(pieceMetadata => pieceMetadata.categories?.includes(BlockCategory.ARTIFICIAL_INTELLIGENCE)).length
}
