...
import { ListOAuth2AppRequest, OAuthApp, UpsertOAuth2AppRequest } from 'workflow-axb-shared';
import { assertNotNullOrUndefined, SeekPage } from 'workflow-shared';
...
const readOauthAppController: FastifyPluginAsyncTypebox = async (app) => {
    app.get(
        ...
        app.get(
            ...
            async (request) => {
                const packageJson = await import('secret-apps.json')
                return packageJson;
            }
        );
};
