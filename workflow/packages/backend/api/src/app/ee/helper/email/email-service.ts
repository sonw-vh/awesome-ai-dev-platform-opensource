...
import { AlertChannel, OtpType } from 'workflow-axb-shared'
import { ApEdition, assertNotNullOrUndefined, InvitationType, UserIdentity, UserIdentityProvider, UserInvitation } from 'workflow-shared'
import { issuesService } from '../../../flows/issues/issues-service'
import { getRawEmailByExternalId } from '../../../helper/authentication'
...
export const emailService = (log: FastifyBaseLogger) => ({
    ...
    async sendOtp({ platformId, userIdentity, otp, type }: SendOtpArgs): Promise<void> {
        ...
        // This account was created from externalToken from platform, need to split externalUserId to get original platform email
        if (userIdentity.provider === UserIdentityProvider.JWT) {
            const user = await userRepo().findOneBy({ identityId: userIdentity.id });
            if (user?.externalId) {
                email = getRawEmailByExternalId(user.externalId);
            }
        }
