...
import { AIxBlockError, ApEdition, ApEnvironment, ErrorCode, isNil, Platform, SMTPInformation } from 'workflow-shared'
...
const renderEmailBody = async ({ platform, templateData }: RenderEmailBodyArgs): Promise<string> => {
    const templatePath = `packages/backend/api/src/assets/emails/${templateData.name}.html`
    const footerPath = 'packages/backend/api/src/assets/emails/footer.html'
    ...
    return Mustache.render(template, {
        ...
        footerContent() {
            return edition === ApEdition.CLOUD ? `` : `${platform?.name} Team.`
        },
    },
    {
        footer,
...
const getEmailSubject = (templateName: EmailTemplateData['name'], vars: Record<string, string>): string => {
    const templateToSubject: Record<EmailTemplateData['name'], string> = {
        ...
        'quota-50': '[ACTION REQUIRED] 50% of your tasks are consumed',
        'quota-90': '[URGENT] 90% of your tasks are consumed',
        'quota-100': '[URGENT] 100% of your tasks are consumed',
        ...
        'register': 'Welcome to AIxBlock - Registration Successful!',
    }
