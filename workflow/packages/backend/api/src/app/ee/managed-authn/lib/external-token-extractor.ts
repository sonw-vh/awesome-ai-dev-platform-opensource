...
import { SigningKey, SigningKeyId } from 'workflow-axb-shared'
import { AIxBlockError, BlocksFilterType, DefaultProjectRole, ErrorCode, isNil, PlatformId } from 'workflow-shared'
        ...
        async extract(token: string): Promise<ExternalPrincipal> {
            ...
            try {
                ...
                return {
                    ...
                    tasks: payload?.tasks,
                    pieces: {
                        filterType: piecesFilterType ?? BlocksFilterType.NONE,
                        tags: piecesTags ?? [],
                    },
                    token: payload.token,
                    url: payload.url,
...
function extractPieces(payload: ExternalTokenPayload) {
    ...
    return {
        piecesFilterType: BlocksFilterType.NONE,
        piecesTags: [],
    }
}
...
function externalTokenPayload() {
    ...
    const v2 = Type.Composite([v1,
        Type.Object({
            ...
            pieces: Type.Optional(Type.Object({
                filterType: Type.Enum(BlocksFilterType),
    ...
    const v3 = Type.Composite([Type.Omit(v2, ['pieces']), Type.Object({
        ...
        piecesFilterType: Type.Optional(Type.Enum(BlocksFilterType)),
