...
import { BlocksSource, fileExists, getEngineTimeout } from 'workflow-server-shared'
import { assertNotNullOrUndefined, EngineOperation, EngineOperationType, EngineResponse, EngineResponseStatus } from 'workflow-shared'
...
export class IsolateSandbox {
    ...
    private static runIsolate(cmd: string): Promise<string> {
        ...
        const fullCmd = `${currentDir}/packages/backend/api/src/assets/${this.isolateExecutableName} ${cmd}`
    ...
    private getEnvironmentVariables(): Record<string, string> {
        ...
        return {
            ...
            AP_BLOCKS_SOURCE: workerMachine.getSettings().BLOCKS_SOURCE,
   ...
   */
    private getDirsToBindArgs(): string[] {
        const etcDir = path.resolve('./packages/backend/api/src/assets/etc/')
        ...
        const piecesSource = workerMachine.getSettings().BLOCKS_SOURCE

        if (piecesSource === BlocksSource.FILE) {
