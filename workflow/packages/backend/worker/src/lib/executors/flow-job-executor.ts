...
import { exceptionHandler, OneTimeJobData, pinoLogging } from 'workflow-server-shared'
import { AIxBlockError, BeginExecuteFlowOperation, ErrorCode, ExecutionType, FlowRunStatus, FlowVersion, GetFlowVersionForWorkerRequestType, isNil, ResumeExecuteFlowOperation, ResumePayload } from 'workflow-shared'
    ...
    async executeFlow(jobData: OneTimeJobData, engineToken: string): Promise<void> {
        try {
            ...
            if (result.status === FlowRunStatus.INTERNAL_ERROR) {
                await handleInternalError(jobData, engineToken, new AIxBlockError({
            ...
            const isQuotaExceededError = e instanceof AIxBlockError && e.error.code === ErrorCode.QUOTA_EXCEEDED
            const isTimeoutError = e instanceof AIxBlockError && e.error.code === ErrorCode.EXECUTION_TIMEOUT
            const isMemoryIssueError = e instanceof AIxBlockError && e.error.code === ErrorCode.MEMORY_ISSUE
