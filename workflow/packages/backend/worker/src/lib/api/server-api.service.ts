...
import { ApQueueJob, exceptionHandler, GetRunForWorkerRequest, PollJobRequest, QueueName, ResumeRunRequest, SavePayloadRequest, SendEngineUpdateRequest, SubmitPayloadsRequest, UpdateFailureCountRequest, UpdateJobRequest } from 'workflow-server-shared'
import { AIxBlockError, ErrorCode, FlowRun, GetBlockRequestQuery, GetFlowVersionForWorkerRequest, PopulatedFlow, RemoveStableJobEngineRequest, UpdateRunProgressRequest, WorkerMachineHealthcheckRequest, WorkerMachineHealthcheckResponse } from 'workflow-shared'
...
export const engineApiService = (engineToken: string, log: FastifyBaseLogger) => {
    ...
    return {
        ...
        async getPiece(name: string, options: GetBlockRequestQuery): Promise<BlockMetadataModel> {
            return client.get<BlockMetadataModel>(`/v1/blocks/${encodeURIComponent(name)}`, {
                params: options,
            })
        },
