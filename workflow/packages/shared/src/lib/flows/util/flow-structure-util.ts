...
function extractRequiredValues(objects: any[]) {
    if (!objects || !Array.isArray(objects)) return { isInvalid: false, obj: [] };
    try {
        ...
        return { isInvalid, datasource: obj };
    } catch {
        return { isInvalid: false, obj: [] };
    }
}
