...
const removeAuthFromProps = (props: Record<string, any>): Record<string, any> => {
...
const PieceSettings = React.memo((props: PieceSettingsProps) => {
    ...
    const { data: webhookPrefixUrl } = flagsHooks.useFlag<string>(ApFlagId.WEBHOOK_URL_PREFIX);

    const { data: pausedFlowTimeoutDays } = flagsHooks.useFlag<number>(ApFlagId.PAUSED_FLOW_TIMEOUT_DAYS);

    const { data: webhookTimeoutSeconds } = flagsHooks.useFlag<number>(ApFlagId.WEBHOOK_TIMEOUT_SECONDS);
    ...
    const markdownVariables = {
        ...
        aixblockFormUrl: `${frontendUrl}aixblock-web-forms/${props.flowId}`,
        aixblockActionFormUrl: `${frontendUrl}aixblock-web-forms/${props.flowId}${
            props.step.name !== 'trigger' ? `?stepName=${props.step.name}&flowRunId=${props.flowRunId}&` : '?'
        }`,
        aixblockListTasksUrl: `${frontendUrl}aixblock-tasks/${props.flowId}${
            props.step.name !== 'trigger' ? `?stepName=${props.step.name}&flowRunId=${props.flowRunId}&multimodalKey=${props.step.settings.input.multimodalKey}&` : '?'
        }`,
        aixblockCustomMultimodalUrl: `${frontendUrl}aixblock-tasks/custom-multimodal/${props.flowId}${
            props.step.name !== 'trigger' ? `?stepName=${props.step.name}&flowRunId=${props.flowRunId}&multimodalKey=${props.step.settings.input.storeKey}&` : '?'
        }`,
            ...
            {pieceModel && (
                <>
                    {pieceModel.auth && (showAuthForAction || showAuthForTrigger) && (
                        <ConnectionSelect isTrigger={!isNil(selectedTrigger)} piece={pieceModel} disabled={props.readonly}></ConnectionSelect>
                    )}
                    {selectedAction && (
