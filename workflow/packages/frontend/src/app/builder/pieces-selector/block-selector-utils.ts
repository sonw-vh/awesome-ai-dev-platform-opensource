...
function toKey(stepMetadata: StepMetadata): string {
  switch (stepMetadata.type) {
    ...
    case TriggerType.PIECE: {
      ...
      return `${stepMetadata.type}-${pieceMetadata.blockName}-${pieceMetadata.pieceVersion}`;
    }
    default:
...
const isCorePiece = (piece: StepMetadata) =>
  piece.type !== TriggerType.PIECE && piece.type !== ActionType.PIECE
    ? true
    : (piece as PieceStepMetadata).categories.includes(BlockCategory.CORE);
...
const isAiPiece = (piece: StepMetadata) =>
  piece.type === TriggerType.PIECE || piece.type === ActionType.PIECE
    ? piece.categories.includes(BlockCategory.ARTIFICIAL_INTELLIGENCE)
    : false;

const isAppPiece = (piece: any) =>
  (!isAiPiece(piece) && !isCorePiece(piece)) || piece.blockName === 'workflow-aixblock';
...
const isPopularPieces = (
  ...
  return [...popularPieces, ...pinnedPieces].includes(
    (stepMetadata as PieceStepMetadata).blockName,
  );
};
...
const isUniversalAiPiece = (stepMetadata: StepMetadata) => {
  if (stepMetadata.type === ActionType.PIECE) {
    return stepMetadata.categories.includes(BlockCategory.UNIVERSAL_AI);
  }
  return false;
};
      ...
      return deepMergeAndCast<PieceAction>(
        {
          ...
          settings: settings ?? {
            blockName: stepMetadata.blockName,
            blockType: stepMetadata.blockType,
