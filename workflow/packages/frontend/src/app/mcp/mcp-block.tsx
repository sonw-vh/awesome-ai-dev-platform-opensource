...
import { PlugIcon, RotateCcw, Trash2, Unplug } from 'lucide-react';
...
type McpPieceProps = {
  piece: McpBlockWithConnection;
  pieceInfo: {
    displayName: string;
    logoUrl?: string;
  };
  onDelete: (piece: McpBlockWithConnection) => void;
  isLoading?: boolean;
};
...
export const McpPiece = ({
  ...
  const { mutate: updatePieceStatus } = useMutation({
    ...
    onSuccess: (status) => {
      toast({
        ...
        description: t(
          `${
            status === McpBlockStatus.ENABLED ? 'Enabled' : 'Disabled'
          } successfully`,
        ),
      });
      refetchMcp();
  ...
  const [status, setStatus] = useState(piece.status === McpBlockStatus.ENABLED);
        ...
        <div className="flex items-center gap-1 flex-shrink-0 ml-2">
          <Switch
            ...
            onCheckedChange={(checked) => {
              updatePieceStatus(
                checked ? McpBlockStatus.ENABLED : McpBlockStatus.DISABLED,
              );
            }}
            className="scale-75"
          />
