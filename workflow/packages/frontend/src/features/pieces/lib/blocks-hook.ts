...
import { CORE_STEP_METADATA, piecesApi } from './blocks-api';
import {
  PieceSelectorItem,
  StepMetadata,
...
function stepMetadataQueryBuilder(step: Step) {
  ...
  const blockName = isPieceStep ? step.settings.blockName : undefined;
  const pieceVersion = isPieceStep ? step.settings.pieceVersion : undefined;
  const customLogoUrl =
    'customLogoUrl' in step ? step.customLogoUrl : undefined;
  return {
    queryKey: ['piece', step.type, blockName, pieceVersion, customLogoUrl],
