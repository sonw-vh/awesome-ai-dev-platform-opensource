...
import { ApEdition, ApFlagId, AuthenticationResponse, ErrorCode, isNil, SignInRequest } from 'workflow-shared';

const SignInSchema = Type.Object({
    email: Type.String({
        minLength: 1,
        errorMessage: t('Email or username is required'),
    }),
...
const SignInForm: React.FC = () => {
    ...
    const { mutate, isPending } = useMutation<AuthenticationResponse, HttpError, SignInRequest>({
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem className="grid space-y-2">
                            ...
                            <Input {...field} required id="email" type="text" placeholder={'email@example.com'} className="rounded-sm" tabIndex={1} />
                            <FormMessage />
                        </FormItem>
                    )}
                />
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem className="grid space-y-2">
                            ...
                            <Input {...field} required id="password" type="password" placeholder={'********'} className="rounded-sm" tabIndex={2} />
                            <FormMessage />
                        </FormItem>
                    )}
                />
                {form?.formState?.errors?.root?.serverError && <FormMessage>{form.formState.errors.root.serverError.message}</FormMessage>}
                <Button loading={isPending} onClick={(e) => form.handleSubmit(onSubmit)(e)} tabIndex={3}>
                    {t('Sign in')}
                </Button>
            </form>
        </Form>
