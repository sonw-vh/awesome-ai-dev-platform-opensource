...
import { InputPropertyMap, PiecePropertyMap, StaticPropsValue, TriggerStrategy } from 'workflow-blocks-framework'
import { assertEqual, assertNotNullOrUndefined, AUTHENTICATION_PROPERTY_NAME, EventPayload, ExecuteTriggerOperation, ExecuteTriggerResponse, isNil, PieceTrigger, ScheduleOptions, Trigger, TriggerHookType } from 'workflow-shared'
...
export const triggerHelper = {
    async executeOnStart(trigger: Trigger, constants: EngineConstants, payload: unknown) {
        const { blockName, pieceVersion, triggerName, input, inputUiInfo } = (trigger as PieceTrigger).settings
    ...
    async executeTrigger({ params, constants }: ExecuteTriggerParams): Promise<ExecuteTriggerResponse<TriggerHookType>> {
        const { blockName, pieceVersion, triggerName, input, inputUiInfo } = (params.flowVersion.trigger as PieceTrigger).settings
        assertNotNullOrUndefined(triggerName, 'triggerName is required')
            ...
            case TriggerHookType.RUN: {
                if (pieceTrigger.type === TriggerStrategy.APP_WEBHOOK) {
                    if (!params.appWebhookUrl) {
                        throw new Error(`App webhook url is not available for piece name ${blockName}`)
                    }
                    if (!params.webhookSecret) {
                        throw new Error(`Webhook secret is not available for piece name ${blockName}`)
                    }
...
async function prepareTriggerExecution({ blockName, pieceVersion, triggerName, input, projectId, apiUrl, engineToken, piecesSource, inputSchema }: PrepareTriggerExecutionParams) {
    const { piece, pieceTrigger } = await blockLoader.getBlockAndTriggerOrThrow({
        blockName,
        pieceVersion,
        triggerName,
        piecesSource,
