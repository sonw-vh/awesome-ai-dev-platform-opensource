...
import { URL } from 'url'
import { ActionContext, ConnectionsManager, InputPropertyMap, PauseHook, PauseHookParams, PiecePropertyMap, RespondHook, RespondHookParams, StaticPropsValue, StopHook, StopHookParams, TagsManager } from 'workflow-blocks-framework'
import { ActionType, assertNotNullOrUndefined, AUTHENTICATION_PROPERTY_NAME, ExecutionType, FlowRunStatus, GenericStepOutput, isNil, PauseType, PieceAction, RespondResponse, StepOutputStatus } from 'workflow-shared'
...
const executeAction: ActionHandler<PieceAction> = async ({ action, executionState, constants }) => {
    ...
    try {
        ...
        const { pieceAction, piece } = await blockLoader.getBlockAndActionOrThrow({
