...
export const RegionTags = ({
  ...
  return regions
    ...
    .map((region) => {
      ...
      const { canvasHeight } = layoutParams.current
      let margin = 8
      ...
      const isOutsideBottom = pbox.y + pbox.h + labelBoxHeight > canvasHeight;
      const offsetTop = isOutsideBottom ? canvasHeight - pbox.y - pbox.h - labelBoxHeight : 0;

      const coords = displayOnTop
        ? {
            left: Math.max(pbox.x, 0),
            top: pbox.y - margin / 2,
          }
        : { left: Math.max(pbox.x, 0), top: pbox.y + pbox.h + (margin / 2) + offsetTop }
      if (region.locked) {
        return (
          <div
            key={region.id}
