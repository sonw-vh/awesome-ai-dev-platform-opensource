...
export default (state: MainLayoutVideoAnnotationState, action: Action) => {
  const copyImpliedRegions = () => {
    console.log(state.currentVideoTime);
    return setIn(
      setIn(saveToHistory(state, "Add Keyframe"), ["saveState"], "start"),
  ...
  switch (action.type) {
    ...
    case "DELETE_KEYFRAME": {
      return setIn(
        setIn(state, ["saveState"], "start"),
        ["keyframes"],
        without(state.keyframes, action.time)
      )
    }
    default:
      break
