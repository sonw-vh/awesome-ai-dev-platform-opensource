...
const Model = types.model({
  ...
  toggleSelected() {
    ...
    if (self.selected) {
      const target = self.annotation.names.get(labels.toname);

      if (target && "triggerAfterLabelSelectedForRichText" in target) {
        target.triggerAfterLabelSelectedForRichText();
      }
    }
  },
  ...
  observer(({ item, store }) => {
    ...
    const label = (
      <Label color={item.background} margins empty={item.isEmpty} hotkey={hotkey} hidden={!item.visible} selected={item.selected} onClick={() => {
        if (!item.selected && item.annotationStore.selected.regionStore && !item.annotationStore.selected.regionStore.hasSelection) {
          const obj = item.annotationStore.selected.objects[0];

          if (obj?.type === "image") {
            const result = item.annotationStore.selected.createResult(
              {
                coordstype: "perc",
                dynamic: false,
                x: 0,
            ...
            store.annotationStore.toNames.forEach(name => {
              if (name.length > 0) {
                name.forEach(n => {
                  if (n?.type !== "textarea") return;
            ...
            result.notifyDrawingFinished();
            item.annotationStore.selected.selectArea(result);
          }
        }

        item.toggleSelected();
        return false;
      }} onDelete={store.canCreateLabel ? (e) => {
        e.stopPropagation();

        confirm({
          title: "Delete Label",
          body: "Are you sure you want to delete this label?",
          onOk: () => item.parent.deleteLabel(item.value),
        });
      } : undefined}>
