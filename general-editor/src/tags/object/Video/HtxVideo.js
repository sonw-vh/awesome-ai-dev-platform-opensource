...
const HtxVideoView = ({ store, item }) => {
  ...
  const rs = store.annotationStore.selected?.regionStore;
  ...
  const paginatedRegIds = useMemo(() => rs.paginatedRegions.map(r => r.id), [rs.paginatedRegions]);
  const selectedRegIds = useMemo(() => rs.selectedIds, [rs.selectedIds]);

  const visibleRegs = useMemo(
    () => [
      ...rs.paginatedRegions,
      ...rs.sortedRegions.filter(
        r => !paginatedRegIds.includes(r.id)
          && (
            selectedRegIds.includes(r.id)
            || !!r.sequence.find(s => s.frame === item.frame || (s.frame < item.frame && s.enabled === true))
          ),
      ),
    ],
    [rs.paginatedRegions, rs.sortedRegions, selectedRegIds, item.frame],
  );
  ...
  const handleAction = useCallback((_, action, data) => {
    const regions = visibleRegs.filter(reg => reg.selected || reg.inSelection);
  ...
  const regions = playing ? [] : visibleRegs.map(reg => {
    ...
    const customIdx = paginatedRegIds.includes(reg.id) ? undefined : rs.sortedRegions.indexOf(reg);
    ...
    return {
      ...
      sequence,
      idx: customIdx ? customIdx + 1 : undefined,
    };
  });
      ...
      <Block name="video-segmentation" ref={mainContentRef} mod={{ fullscreen: isFullScreen }}>
        ...
        {loaded && (
          <Elem
            ...
            position={position}
            regions={regions}
            paginatedRegIds={paginatedRegIds}
            ...
            idxOffset={rs ? (rs.page - 1) * rs.pageSize : 0}
          />
        )}
      </Block>
    </ObjectTag>
