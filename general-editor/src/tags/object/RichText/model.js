...
const Model = types
  ...
  .actions(self => {
    let beforeNeedsUpdateCallback, afterNeedsUpdateCallback, afterLabelSelected;

    return {
      triggerAfterLabelSelectedForRichText() {
        afterLabelSelected?.();
      },

      setAfterLabelSelected(cb) {
        afterLabelSelected = cb;
      },
      ...
      unselect() {
        getRoot(self).annotationStore.selected?.unselectAll();
      },
    };
  });
