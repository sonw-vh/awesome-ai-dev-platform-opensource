...
export const LabelList = observer(({ regionStore }) => {
  const idxOffset = useMemo(() => {
    return (regionStore.page - 1) * regionStore.pageSize;
  }, [regionStore.page, regionStore.pageSize]);
  ...
  const treeData = regionStore.asLabelsTree((item, idx, isLabel, children, onClick) => {
    return {
      ...
      title: (data) => {
        return isLabel ? (
          <LabelItem item={item} idx={idxOffset + idx} regions={data.children} regionStore={regionStore} />
        ) : (
          <RegionItem item={item} idx={idxOffset + idx} onClick={onClick}/>
        );
      },
    };
  }, true);
