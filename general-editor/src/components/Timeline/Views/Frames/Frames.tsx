...
export const Frames: FC<TimelineViewProps> = ({
  ...
  const background = useMemo(() => {
    const bg = [
      `repeating-linear-gradient(90deg, #fff 1px, #fff ${step-1}px, rgba(0,0,0,0) ${step-1}px, rgba(0,0,0,0) ${step+1}px)`,
      `linear-gradient(0deg, #f0f0f0, rgba(0,0,0,0) 50%)`,
    ];
  ...
  return (
    <Elem name="keypoints" style={{ height: regions.length * height }}>
      {regions.map((region, i) => {
        ...
        if (paginatedRegIds.includes(region.originalId)) {
          counter++;
          idx = idxOffset + counter;
        }
