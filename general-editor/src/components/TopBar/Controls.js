      ...
      buttons.push(
        <ButtonTooltip key="submit" title={title}>
          <Elem name="tooltip-wrapper">
            <Button aria-label="submit" className="lsf-controls--submit" disabled={disabled || submitDisabled} look="primary" onClick={async () => {
              await store.commentStore.commentFormSubmit();
    ...
    if ((userGenerate && sentUserGenerate) || (!userGenerate && store.hasInterface("update"))) {
      ...
      const button = (
        <ButtonTooltip key="update" title="Update this task: [ Alt+Enter ]">
          <Button className="lsf-controls--submit" aria-label="submit" disabled={disabled || submitDisabled} look="primary" onClick={async () => {
            await store.commentStore.commentFormSubmit();
