...
function attrsToProps(node: Element, replaces?: Record<string, string>): Record<string, any> {
  ...
  for (const attr of node.attributes) {
    ...
    const checkBoolValue =
      (["chat"].includes(lowerTagName) && name === "canAdd")
      || (["aiconfigboolean"].includes(lowerTagName) && name === "value")
      || (["aiconfigstring"].includes(lowerTagName) && name === "multiline");

    if (checkBoolValue || (name !== "value" && ["true", "false"].includes(value))) {
 ...
 * @param {*} item
 */
function renderChildren(item: IAnyStateTreeNode, filterLabel = "") {
  if (item && item.children && item.children.length) {
    const _label = filterLabel.trim().toLowerCase();
    ...
    return item.children.map((el: IAnyStateTreeNode) => {
      if (el.type === "label" || el.type === "choice") {
        if (el.value?.toLowerCase().indexOf(_label) === -1) {
          return null;
        }
      }
