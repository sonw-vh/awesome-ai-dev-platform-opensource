...
export default types.model("RegionStore", {
  ...
  pageSize: types.optional(types.integer, 100),
}).views(self => {
  let lastClickedItem;
  ...
  return{
    ...
    get hasPagination() {
      return self.annotation.areas.size > self.pageSize;
    },

    get pageCount() {
      return Math.ceil(self.annotation.areas.size / self.pageSize);
    },

    get classifications() {
      if (!isAlive(self.annotation.names)) {
        return [];
      }
    ...
    get regions() {
      ...
      //   return self.paginatedRegions;
      // }

      return Array.from(self.annotation.areas.values()).filter(area => !area.classification);
    },

    get paginatedRegions() {
      let counter = -1;
      const list = [];
      const pageStart = (self.page - 1) * self.pageSize;
      const pageEnd = self.page * self.pageSize;
      const entries = self.sortedRegions.entries();
    ...
    getRegionsTree(enrich, paginated = false) {
      if (self.group === null || self.group === "manual") {
        return self.asTree(enrich, paginated);
      } else if (self.group === 'label') {
        return self.asLabelsTree(enrich, paginated);
      } else if (self.group === 'type') {
        return self.asTypeTree(enrich, paginated);
      } else {
        console.error(`Grouping by ${self.group} is not implemented`);
      }
    },

    asTree(enrich, paginated = false) {
      const regions = paginated ? self.paginatedRegions : self.sortedRegions;
      const tree = [];
    ...
    asLabelsTree(enrich, paginated = false) {
      ...
      const regs = paginated ? self.paginatedRegions : self.regions;
    ...
    asTypeTree(enrich, paginated = false) {
      // collect all label states into two maps
      const groups = {};
      const result = [];
      const onClick = createClickRegionInTreeHandler(result);
  ...
  selectNext() {
    const { regions } = self;
    const idx = self.sortedRegions.findIndex(r => r.selected);
  ...
  highlight(area) {
    // console.log(self.indexFromSortedOf(area.id));
    self.selection.highlight(area);
  },

  indexFromSortedOf(regionId) {
    const entries = self.sortedRegions.entries();
    let next;
