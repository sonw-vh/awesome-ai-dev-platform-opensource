...
export default types
  ...
  .views(self => ({
    ...
    get isQuestionsExtract() {
      let result = true;

      Array.from(self.names.values()).forEach(n => {
        if (["audio", "hypertext", "image", "list", "paragraphs", "table", "text", "timeseries", "transcription", "video"].includes(n.type)) {
          result = false;
        }
      });
