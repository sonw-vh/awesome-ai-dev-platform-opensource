...
export const Annotation = types
  ...
  .actions(self => ({
    ...
    deserializeResults(json, { suggestions = false, hidden = false } = {}) {
      ...
      const events = getEnv(self.store).events;
      ...
      try {
        ...
        objAnnotation.forEach((obj, idx) => {
          ...
          const newPercent = (idx + 1) / objAnnotation.length * 100;

          if (Math.floor(newPercent) > currentPercent) {
            events.invoke("processValueProgress", newPercent);
        ...
        objAnnotation.forEach((obj, idx) => {
          ...
          if (Math.floor(newPercent) > currentPercent) {
            events.invoke("processRelationProgress", newPercent);
            currentPercent = Math.floor(newPercent);
          }
        });
      } catch (e) {
    ...
    async redact() {
      await getEnv(self).events.invoke('redactAnnotation', self);
    },
  }));
