...
Scenario("Basic scenario", async ({ I, AIxBlock, AtImageView, AtSettings, AtLabels, AtSidebar }) => {
  I.amOnPage("/");
...
Scenario("Drawing multiple blank regions and then attaching labels", async ({ I, AIxBlock, AtImageView, AtSettings, AtLabels, AtSidebar }) => {
  I.amOnPage("/");
  AIxBlock.init({ config: createConfig( ), data });
  AtImageView.waitForImage();
  AtSettings.open();
  AtSettings.setGeneralSettings({
    [AtSettings.GENERAL_SETTINGS.SHOW_LABELS]: true,
  ...
  session('Deserialization', () => {
    I.amOnPage('/');
    AIxBlock.init({ config: createConfig( ), data, annotations: [{ id: "test", result: results }] });
    AtImageView.waitForImage();
    AtSidebar.seeRegions(regions.length);
    for (const [idx, region] of Object.entries(regions)) {
      if (region.text) {
