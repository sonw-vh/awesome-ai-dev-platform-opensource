...
const data = {
  url: "https://htx-misc.s3.amazonaws.com/opensource/aixblock/examples/audio/barradeen-emotional.mp3",
  text: [
    {
      "end": 5.6,
...
Scenario("Check audio clip is played when using the new sync option", async function({ I, AIxBlock, AtAudioView }) {

  I.amOnPage("/");

  const hasFFDev1713 = await AIxBlock.hasFF("ff_front_DEV_1713_audio_ui_150222_short");
  const hasFFDev2461 = await AIxBlock.hasFF("fflag_feat_front_dev_2461_audio_paragraphs_seek_chunk_position_short");
