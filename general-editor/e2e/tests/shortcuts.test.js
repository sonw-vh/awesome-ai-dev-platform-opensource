...
const AUDIO_URL = "https://htx-misc.s3.amazonaws.com/opensource/aixblock/examples/audio/barradeen-emotional.mp3";

const TEXT_SELECTOR = "[name='text']";

Data(configParams).Scenario("Should keep the focus and cursor position.", async ({ I, AIxBlock, AtSidebar, current }) => {
...
Data(configParams).Scenario("Should work with emoji.", async ({ I, AIxBlock, AtSidebar, current }) => {
  const { inline } = current;
  const config = createConfig({
    rows: inline ? "1" : "3",
  });
...
Data(configParams).Scenario("Should work with existent regions.", async ({ I, AIxBlock, AtSidebar, current }) => {
  const { inline } = current;
  const config = createConfig({
    rows: inline ? "1" : "3",
  });
