...
examples.forEach(example => {
  ...
  Scenario(`Nonexistent label -> ${title}`, async ({ I, AIxBlock, AtSidebar, AtImageView, AtAudioView }) => {
    ...
    if (regionsCount) {
      const restored = await AIxBlock.serialize();
  ...
  Scenario(`Different from_name -> ${title}`, async ({ I, AIxBlock, AtSidebar, AtImageView, AtAudioView }) => {
    let { result = annotations[0].result } = example;
  ...
  Scenario(`Nonexistent from_name -> ${title}`, async ({ I, AIxBlock, AtTopbar, AtSidebar }) => {
    const params = { annotations: [{ id: "test", result }], data };
    const configTree = Utils.parseXml(config);
...
[SINGLE_TYPE, MULTIPLE_TYPE].forEach(type => {
  Scenario(`Making labels empty -> choice="${type}"`, async ({ I, AIxBlock, AtSidebar, AtAudioView, AtLabels }) => {
...
Scenario(`Consistency of empty labels`, async ({ I, AIxBlock, AtSidebar, AtImageView, AtLabels }) => {
