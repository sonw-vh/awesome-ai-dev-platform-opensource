...
const IMAGE =
  "https://htx-misc.s3.amazonaws.com/opensource/aixblock/examples/images/nick-owuor-astro-nic-visuals-wDifg5xc9Z4-unsplash.jpg";
...
Data(shapesTable).Scenario("Simple rotation", async function({ I, AIxBlock, AtImageView, AtSidebar, current }) {
...
Data(shapesTable).Scenario("Rotate zoomed", async function({ I, AIxBlock, AtImageView, AtSidebar, current }) {
  const params = {
...
Data(windowSizesTable).Scenario("Rotation with different window sizes", async function({ I, AIxBlock, AtImageView, AtSidebar, current }) {
...
Data(layoutVariations).Scenario("Rotation in the two columns template", async function({ I, AIxBlock, AtImageView, AtSidebar, AtSettings, current }) {
  I.amOnPage("/");
