...
Data(shapesTable).Scenario("Check transformer existing for different shapes, their amount and modes.", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
  ...
  AIxBlock.init(getParamsWithLabels(shapeName));
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMoveToolTransformer))
  .Scenario("Resizing a single region", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];

    I.amOnPage("/");
    AIxBlock.init(getParamsWithShape(shapeName, Shape.params));
    AtImageView.waitForImage();
    AtSidebar.seeRegions(0);
    await AtImageView.lookForStage();
    ...
    const rectangleResult = await AIxBlock.serialize();
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionRotator))
  .Scenario("Simple rotating", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionRotator))
  .Scenario("Rotating of unrotatable region", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionRotator))
  .Scenario("Broke the limits with rotation", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...

    {
      ...
      let rectangleResult = await AIxBlock.serialize();
      ...
      rectangleResult = await AIxBlock.serialize();
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionRotator))
  .Scenario("Check the initial rotation of transformer for the single region", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionRotator))
  .Scenario("Check the initial rotation of transformer for the couple of regions", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionTransformer && shapeName !== "KeyPoint"))
  .Scenario("Transforming of multiple regions", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionTransformer))
  .Scenario("Move regions by drag", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionRotator))
  .Scenario("Limitation of dragging a single rotated region", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasMultiSelectionRotator))
  .Scenario("Limitation of dragging a couple of rotated regions", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
...
Data(shapesTable.filter(({ shapeName }) => shapes[shapeName].hasRotator))
  .Scenario("Rotating the region near the border", async ({ I, AIxBlock, AtImageView, AtSidebar, current }) => {
    const { shapeName } = current;
    const Shape = shapes[shapeName];
