...
const { initAIxBlock, countKonvaShapes, switchRegionTreeView } = require("./helpers");
...
Scenario("Checking mass toggling of visibility", async ({ I, AtImageView, AtSidebar }) => {
  ...
  await I.amOnPage("/");
  I.executeScript(initAIxBlock, { annotations, config, data });
...
Scenario("Hiding bulk visibility toggle", ({ I, AtImageView, AtLabels, AtSidebar }) => {
  I.amOnPage("/");
  I.executeScript(initAIxBlock, { config, data });
  AtImageView.waitForImage();
