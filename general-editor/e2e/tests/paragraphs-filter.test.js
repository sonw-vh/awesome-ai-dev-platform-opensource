...
const AUDIO = "https://htx-misc.s3.amazonaws.com/opensource/aixblock/examples/audio/barradeen-emotional.mp3";
...
Scenario("Create two results using excluding a phrase  by the filter", async ({ I, AIxBlock, AtSidebar, AtParagraphs, AtLabels }) => {
  const params = {
    data: DATA,
    config: CONFIG,
  };
  ...
  AIxBlock.setFeatureFlags(FEATURE_FLAGS);
  ...
  const twoActionsResult = AIxBlock.serialize();
  ...
  const oneActionResult = AIxBlock.serialize();
...
Scenario("Check different cases ", async ({ I, AIxBlock, AtSidebar, AtParagraphs, AtLabels }) => {
  const dialogue = [
    1,// 1
    3,// 2
    1,// 3
