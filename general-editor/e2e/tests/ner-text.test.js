...
const { initAIxBlock, serialize, selectText } = require("./helpers");
...
Scenario("NER Text with SECURE MODE and wrong valueType", async function({ I, AIxBlock }) {
  const params = {
    annotations: [{ id: "TestCmpl", result: results }],
    config: configSimple,
    data: { url },
...
Scenario("NER Text with SECURE MODE", async function({ I, AIxBlock }) {
  const params = {
    annotations: [{ id: "TestCmpl", result: resultsFromUrl }],
    config: configUrl,
    data: { url },
