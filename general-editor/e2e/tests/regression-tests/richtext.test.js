...
Scenario("Broken limits", async ({ I, AIxBlock, AtSidebar, AtRichText, ErrorsCollector }) => {
  I.amOnPage("/");
...
Scenario("The selection in degenerate cases", async ({ I, AIxBlock, AtSidebar, AtRichText, ErrorsCollector }) => {
  I.amOnPage("/");
  await ErrorsCollector.run();
  AIxBlock.init({
...
Scenario("Exactly 1 word", async ({ I, AIxBlock, AtSidebar, AtRichText, ErrorsCollector }) => {
  I.amOnPage("/");
  await ErrorsCollector.run();
  AIxBlock.init({
    annotations: [
...
Scenario("Trim spaces around the word", async ({ I, AIxBlock, AtSidebar, AtRichText, ErrorsCollector }) => {
  I.amOnPage("/");
  await ErrorsCollector.run();
  AIxBlock.init({
    annotations: [
  ...
  AtSidebar.see("two");
  const result = await AIxBlock.serialize();
...
Scenario("Trim spaces with BRs", async ({ I, AIxBlock, AtSidebar, AtRichText, ErrorsCollector }) => {
  I.amOnPage("/");
  await ErrorsCollector.run();
  AIxBlock.init({
    annotations: [
...
Scenario("Overlap checks", async ({ I, AIxBlock, AtSidebar, AtRichText, ErrorsCollector }) => {
  I.amOnPage("/");
  await ErrorsCollector.run();
  AIxBlock.init({
    annotations: [
...
Scenario("Non-standard characters in words", async ({ I, AIxBlock, AtSidebar, AtRichText, ErrorsCollector }) => {
  I.amOnPage("/");
  await ErrorsCollector.run();
  AIxBlock.init({
    annotations: [
...
Scenario("Should not select words from next line", async ({ I, AIxBlock, AtSidebar, AtRichText, ErrorsCollector }) => {
  I.amOnPage("/");
  await ErrorsCollector.run();
  AIxBlock.init({
    annotations: [
...
Scenario("Trying to select alt attr", async ({ I, AIxBlock, AtSidebar, ErrorsCollector, AtRichText }) => {
  I.amOnPage("/");
  await ErrorsCollector.run();
  AIxBlock.init({
    annotations: [
