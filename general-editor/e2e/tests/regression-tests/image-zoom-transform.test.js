...
const IMAGE = "https://htx-misc.s3.amazonaws.com/opensource/aixblock/examples/images/nick-owuor-astro-nic-visuals-wDifg5xc9Z4-unsplash.jpg";
...
Scenario("Transforming the region on the border of zoomed image", async ({ I, AIxBlock, AtImageView, AtSidebar }) => {
  const params = {
    config,
    data: { image: IMAGE },
  };
  ...
  const results = await AIxBlock.serialize();

  // The angle of rotation should not exceed 30 degrees
  assert.strictEqual(((results[0].value.rotation + 30) % 360) < 60, true);
});

Scenario("Transforming the region on the border of zoomed image after window resize", async ({ I, AIxBlock, AtImageView, AtSidebar }) => {
  const wWidth = 1200;
  const wHeight = 900;
