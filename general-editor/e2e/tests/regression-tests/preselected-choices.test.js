...
Scenario("Make a duplicate of annotation with preselected choices", async ({ I, AIxBlock, AtSidebar, AtTopbar })=>{
  const params = {
    config: `
<View>
  ...
  const duplicateResult = await AIxBlock.serialize();
  ...
  const annotationWithPresetValues = await AIxBlock.serialize();
...
Scenario("Make a duplicate of empty annotation with preselected choices", async ({ I, AIxBlock, AtSidebar, AtTopbar })=>{
  const params = {
    config: `
<View>
  <Text name="text" value="$text"></Text>
