...
export default class SseEditor3d extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.annotation?.id === this.props.annotation?.id) return;
        this.renderAnnotationResult();
    }
    ...
    updateClassFilter(changedClassIndex) {
        ...
        this.cloudData.forEach((pt, pos) => {
            const visible = classesData[pt.classIndex]?.visible ?? true;
    ...
    createObject() {
        ...
        if (points) {
            ...
            const obj = {
                ...
                classIndex: this.activeClassIndex,
    ...
    componentDidMount() {
        ...
        this.onMsg("rerender-annotation", () => this.renderAnnotationResult());
    }
    ...
    */
    ...
    saveAll() {
        ...
        this.saveTimeout = setTimeout(() => {
            this.sendMsg("save-annotation", {
                labels: this.cloudData.map(x => x.classIndex),
    ...
    renderAnnotationResult() {
        ...
        if (this.props.annotation) {
            const annotationResult = this.props.annotation?.result;

            if (annotationResult && typeof annotationResult === "object" && annotationResult.length > 0) {
                objects = annotationResult[0].objects;

                if (annotationResult[0].labels && annotationResult[0].labels.length > 0) {
                    this.labelArray = annotationResult[0].labels;
                }
            }
        }

        if (this.labelArray.length < this.cloudData.length) {
            for (let i = this.labelArray.length; i < this.cloudData.length; i++) {
                this.labelArray[i] = 0;
            }
        }
        ...
        for (let i = 0; i < this.labelArray.length; i++) {
            ...
            if (this.labelArray[i] < this.props.labels.length) {
                color = this.props.labels[this.labelArray[i]]?.color;
            }

            this.rgbArray.push(SseGlobals.hex2rgb(color));
        }
        ...
        return this.display(
            ...
            .then(async () => {
                ...
                if (this.selectedObject && !this.props.annotation) {
                    this.sendMsg("object-unselect");
                }

                this.sendMsg("maximum-classIndex", { value: this.maxClassIndex });
                this.sendMsg("bottom-right-label", { message: "Ready" });
            });
    }
}
