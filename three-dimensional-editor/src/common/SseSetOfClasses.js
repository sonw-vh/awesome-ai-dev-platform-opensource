...
export default class SseSetOfClasses {
    ...
    labelForIndex(idx) {
        return this._config.objects[idx]?.label ?? "(unknown)";
    }

    colorForIndexAsHex(idx) {
        return this._config.objects[idx]?.color ?? "#FFFFFF";
    }

    colorForIndexAsRGBArray(idx) {
        return this._config.objects[idx]?.color ? SseGlobals.hex2rgb(this._config.objects[idx].color) : "#FFFFFF";
    }
    ...
    propsForIndex(idx) {
        const {label, color} = this._config.objects[idx] ?? {label: "", color: "#FFFFFF"};
        return {label, color};
    }
