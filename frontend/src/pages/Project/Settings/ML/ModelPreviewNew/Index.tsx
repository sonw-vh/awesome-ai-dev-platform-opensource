...
import {convertFLOP, formatFloat, formatGpuMem, formatBytes} from "@/utils/customFormat";
import InputRender from "../InputRender/Index";
import { IAddModelData, getStoreInputFields, storeInputFields } from "../ML";
import "./Index.scss";
import { Checkpoint, ModelSource } from "../helper";
import FormConfig from "@/components/ManageConnect/FormConfig/Index";
import IconPlusSquare from "@/assets/icons/IconPlusSquare";
import { CONFIG_TYPE, Config } from "@/components/ManageConnect/ManageConnect";
...
const ModelPreview = (props: IModelPreviewProps) => {
  ...
  const [precision, setPrecision] = useState<string>(modelConfig?.precision?.toString() || "fp16");
  ...
  const [maxParams, setMaxParams] = useState<number | null>(null);
  ...
  const handlePrecisionChange = (isFP16: boolean) => {
    ...
    storeInputFields({ projectID, precision: isFP16 ? "FP16" : "" })
  };
  ...
  useEffect(() => {
      ...
      ar.promise
        .then(async r => {
          ...
          if (r.ok) {
            ...
            setMaxParams(parseInt(data.paramasters));
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [accuracy, api, batchSize, batchSizePerEpochs, epochs, fps, framework, gpuListIds, imageSize.height, imageSize.width, isAsrType, isCvType, isLlmType, isVideoType, modelID, mono, paramsValue, precision, projectID, resolution, samplingFrequency, setDisableSubmit, token]);
  ...
  return (
    <div className={'c-model-preview'}>
                ...
                return (
                  <div className="c-model-preview__group__detail" key={`key-${index}`}>
                    ...
                    <div>
                      <span>
                        {`Memory: 
                            ${formatBytes(i?.ram)} - Storage: ${formatBytes(i?.disk)} ${i?.ip ? `-  ${i.ip}` : ""}`}
                      </span>
                    </div>
                  </div>
                );
          ...
          <div className={"c-model-preview__row"}>
            <div className={"c-model-preview__input-column flex-none w-50-12"}>
              ...
              <InputBase
                ...
                type="number"
                onBlur={(event) => {
                  handleEstimateCostChange(event);
      ...
      {showEstimateCost && modelPreviewType === "RENT-MODEL" && (
        <div className={"c-model-preview__row"}>
          <div className={"c-model-preview__input-column"}>
