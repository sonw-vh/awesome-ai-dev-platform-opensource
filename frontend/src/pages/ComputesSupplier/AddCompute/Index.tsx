...
const AddCompute = () => {
  ...
  const [time, setTime] = useState<TimeWorking>({
    start_time: "",
    end_time: "",
  });
  ...
  const idCompute = location.pathname.split("/").pop();
  const isEdit = idCompute !== "add";
  ...
  const step = searchParams.get("step") ?? FILL_IP;
  ...
  async function createGpuCompute(
    ...
    try {
      ...
      ar.promise
        .then(async (r) => {
          if (r.ok) {
            navigate(`/computes-supplier/${compute_mkp_id}?step=${PRICING}`);
          } else {
            const res = await r.json();
            if (Object.hasOwn(res, "message")) {
              setError(res.message);
  ...
  React.useEffect(() => {
    ...
    userLayout.setBreadcrumbs([{ label: "Set up computes" }]);
    userLayout.setActions([
      {
        icon: <IconClearCircle />,
        label: "Cancel",
        onClick: () => navigate(`/computes-supplier`),
        actionType: "danger",
      },
    ]);
    return () => {
  ...
  React.useEffect(() => {
    if (isEdit && (step === FILL_IP || step === INSTALL_SH)) {
      setStep(PRICING);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isEdit]);
  ...
  const handleChangeTime = (key: string, value: string) => {
    setTime({ ...time, [key]: value });
  };

  const handleChangeAllDay = (checked: boolean) => {
    setTime({
      ...
      {step === AVAILABILITY ? (
        ...
        <div className="c-add-compute-container-add-host">
          <ComputeForm2
            ...
            onIpPortChange={(ip) => setStep(ip ? INSTALL_SH : FILL_IP)}
            errors={error}
            initData={{
