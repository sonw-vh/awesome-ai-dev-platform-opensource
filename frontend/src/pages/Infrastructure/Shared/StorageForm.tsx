...
import {useSaveGlobalStorage} from "@/hooks/settings/cloudStorage/useSaveGlobalStorage";
...
const StorageForm = ({
  ...
  const {loading: storageTypesLoading, storageTypes} = useGetTypesStorage("storageTypes", true);
  const {loading: saving, error, save, validationErrors} = useSaveGlobalStorage();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [type, setType] = useState<string>(
    isEdit?.storage_type ?? storageTypes?.[0]?.name ?? "s3"
  );
  const {storageFormFields, loading: formLoading} = useGetStorageForm(type, undefined, selectedItem);
  ...
  const handleSubmitForm = async (formData: any) => {
    ...
    save(payloadData, type, selectedItem).promise
      .then(r => {
        if (!r.ok) {
          return;
        }
