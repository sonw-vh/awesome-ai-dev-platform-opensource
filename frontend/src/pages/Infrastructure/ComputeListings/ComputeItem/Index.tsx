...
export default function ComputeItem(props: TProps) {
      ...
      results = [
        {
          ...
          options: props.cp.compute_gpus.map((item: any) => {
            ...
            return {
              label: item?.gpu_name + (price ? " | " + price.price + " " + price.token_symbol + "/" + price.unit : ""),
              value: item?.id,
            }
          }),
        },
  ...
  const status = useMemo(() => {
    if (props.cp?.is_using_cpu) {
      return props.cp?.status === "rented_bought" || props.cp?.status === "in_marketplace"
        ? "Available"
    ...
    if (!props.cp?.is_using_cpu && props.cp?.compute_gpus.length > 0) {
      ...
      return currentCp?.status === "rented_bought" || currentCp?.status === "in_marketplace"
        ? "Available"
  ...
  const defaultCp = useMemo(() => {
    return computeList?.[0]?.options.find(o => o.value === selectedCp);
  }, [computeList, selectedCp]);

  return (
    <div className="p-infrstructure-compute-listings">
      <div className="p-infrstructure-compute-listings__inner">
        <div className="badges">
          ...
          <span className="badge badge--location" id={tooltipPrefix + "location-name"}>
            {currentCp?.location_name ?? "Unknown"}
          </span>
          <Tooltip place="top" positionStrategy="fixed" content="Location"
                   anchorSelect={"#" + tooltipPrefix + "location-name"} />
        ...
        <div className="columns">
          <div className="column column--gpu">
            <Select
              className="p-infrstructure-compute-listings__select"
              data={computeList}
