...
export default function Project() {
  ...
  const createProject = useCallback(() => {
    if (project && project.title === generalData.title && project.color === generalData.color) {
      navigate("/fine-tune-and-deploy/" + project.id + "/setup-infrastructure/storage");
      return;
    }
    ...
    waitingForPromise(
      onCreate(project?.id).then(res => {
        ...
        navigate("/fine-tune-and-deploy/" + res.id + "/setup-infrastructure/storage");
      }),
  ...
  return (
    <FineTuneAndDeployLayout
      onBack={() => {
        if (project) {
          navigate("/fine-tune-and-deploy/" + project.id + "/");
        } else {
          navigate("/fine-tune-and-deploy/");
        }
      }}
