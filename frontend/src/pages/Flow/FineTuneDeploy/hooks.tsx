    ...
    const flows: FlowItem[] = [
      {
        ...
        onClick: project?.id ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/project") : undefined,
      },
      {
        step: 1,
        ...
        isCurrent: pathSegments.length > 3 && pathSegments[3] === "setup-infrastructure",
        onClick: project?.id ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/setup-infrastructure/storage") : undefined,
      },
      {
        step: 2,
        ...
        onClick: project?.id ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/data-preparation/local-upload") : undefined,
      },
      {
        step: 3,
        label: "Set up Models",
        ...
        onClick: project?.id ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/setup-model") : undefined,
      },
      ...project?.data_pipeline === "on" ? [{
        ...
        onClick: project?.id ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/data") : undefined,
      }] : [],
      {
        step: 4,
        label: "Fine-tune",
        ...
        onClick: project?.id ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/training-dashboard") : undefined,
      },
      {
        step: 5,
        ...
        onClick: project?.id ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/demo-and-deploy") : undefined,
      },
    ];
...
export function RequireStorage() {
  ...
  return <CheckStorage whenErrorRedirectTo={"/fine-tune-and-deploy/" + project?.id + "/setup-infrastructure/storage"} />;
}

export function RequireCompute() {
  const {project} = useFlowProvider();
  return <CheckCompute whenErrorRedirectTo={"/fine-tune-and-deploy/" + project?.id + "/setup-infrastructure/gpu"} />;
}

export function RequireModel() {
  const {project} = useFlowProvider();
  return <CheckModel whenErrorRedirectTo={"/fine-tune-and-deploy/" + project?.id + "/setup-model"} />;
}

export function RequireDataPipeline() {
  const {project} = useFlowProvider();
  return <CheckDataPipeline whenErrorRedirectTo={"/fine-tune-and-deploy/" + project?.id + "/settings"} />;
}
